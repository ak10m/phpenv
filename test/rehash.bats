...
@test "creates shims" {
  ...
  assert [ ! -e "${PHPENV_ROOT}/shims/rspec" ]
...
@test "do exact matches when removing stale shims" {
  ...
  cp "$PHPENV_ROOT"/shims/{rspec-core,rspec}
  cp "$PHPENV_ROOT"/shims/{rspec-core,rails}
  cp "$PHPENV_ROOT"/shims/{rspec-core,uni}
  chmod +x "$PHPENV_ROOT"/shims/{rspec,rails,uni}
