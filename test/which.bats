...
@test "outputs path to executable" {
  ...
  PHPENV_VERSION=2.0 run phpenv-which rspec
  assert_success "${PHPENV_ROOT}/versions/2.0/bin/rspec"
}
...
@test "version not installed" {
  create_executable "2.0" "rspec"
  PHPENV_VERSION=1.9 run phpenv-which rspec
...
@test "executable found in other versions" {
  ...
  PHPENV_VERSION=1.8 run phpenv-which rspec
  assert_failure
  assert_output <<OUT
phpenv: rspec: command not found

The \`rspec' command exists in these PHP versions:
  1.9
  2.0
OUT
}
